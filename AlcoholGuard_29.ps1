...
<##
    AlcoholGuard_UI1_ControlPanel_v28.ps1
...
$StabilizationMinimumSeconds = 10
$StabilizationMaximumSeconds = 90
$StabilizationWindowSamples = 100
...
function Start-BreathObservation {
    ...
    # Start from the earliest relevant downward excursion already observed.
    # This prevents a sequence such as 75 -> 60 -> 75 from starting the
    # observation at the recovered value 75 and losing the actual breath signal.
    $seed = @($SeedReadings)
    $startIndex = 0
    ...
    # Seed the observation with the readings from the chosen downward point onward.
    ...
    Write-GuardLog "Breath trigger accepted; starting $BreathObservationSeconds-second downward-trend observation from value=$($script:BreathObservationStartValue) seeded=[$($seed -join ',')]"
}
...
function Process-SensorReading {
    ...
    if ($script:State -eq 'WaitingForBreath') {
        ...
        $downwardHits = @($script:RecentReadings | Where-Object { $_ -le $script:BreathLowerThreshold }).Count
        if ($hits -ge $BreathRequiredHits -and $downwardHits -ge 1) {
...
shell.Run Chr(34) & "$psVbs" & Chr(34) & " -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File " & Chr(34) & "$scriptVbs" & Chr(34) & " -Run$debugArgument", 0, False
Set shell = Nothing
"@
...
function Invoke-SelfTest {
    ...
    foreach ($value in $simSequence) {
        if ($simState -eq 'WaitingForBreath') {
            ...
            if ($simHits -ge $BreathRequiredHits -and $simDownHits -ge 1) {
    ...
    for ($i = 0; $i -lt $StabilizationWindowSamples; $i++) { $stableWindow.Add((@(74,75,74,75)[$i % 4])) }
    ...
    Assert-Test 'Stable 100-sample sensor window is accepted' ($stableWindow.Count -eq 100 -and $stableSpan -le $StabilizationMaxSpan -and $stableDrift -le $StabilizationMaxDrift)
    ...
    Assert-Test 'Stabilization window is 100 samples (~10 seconds)' ($StabilizationWindowSamples -eq 100)
    Assert-Test 'Stabilization drift limit is 3' ($StabilizationMaxDrift -eq 3)
    Assert-Test 'Stabilization requires 10 seconds' ($StabilizationMinimumSeconds -eq 10)
