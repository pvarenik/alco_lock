...
# At baseline 33 this produces a practical breath range of about 20..50.
$BreathUpReferenceDelta = 17.0
$BreathDownReferenceDelta = 13.0

# One out-of-range sample starts the observation; the 5-second directional trend is the confirmation.
$BreathWindowSize = 3
...
function Start-BreathObservation {
    ...
    Write-GuardLog "Breath trigger accepted; starting $BreathObservationSeconds-second $direction-trend observation from value=$($script:BreathObservationStartValue) seeded=[$($seed -join ',')]"
}
...
function Process-SensorReading {
    ...
    if ($script:State -eq 'WaitingForBreath') {
        ...
        # A single out-of-range sample starts a 5-second observation.
...
function Invoke-SelfTest {
    ...
    Assert-Test 'Five-second upward trend confirms sober breath' ($rise -ge $BreathTrendMinimumDrop -and $positiveSteps -ge $BreathTrendMinimumSteps)
    $singleUpSeed = @(49,53,54,56,58,59)
    $singleUpRise = ($singleUpSeed | Measure-Object -Maximum).Maximum - $singleUpSeed[1]
    Assert-Test 'One upward trigger above range can be confirmed over five seconds' ($singleUpSeed[1] -gt $upperThreshold -and $singleUpRise -ge $BreathTrendMinimumDrop)
    ...
    Assert-Test 'Breath deltas are fixed' ($BreathUpReferenceDelta -eq 17 -and $BreathDownReferenceDelta -eq 13)
    Assert-Test 'Baseline 33 uses breath range 20..50' ((33 - $BreathDownReferenceDelta) -eq 20 -and (33 + $BreathUpReferenceDelta) -eq 50)
