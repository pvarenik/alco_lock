...
function Request-Retest {
    if ($script:State -eq 'Unlocked') {
        return
    }

    if (-not $script:SensorConnected -or $null -eq $script:SerialPort -or -not $script:SerialPort.IsOpen) {
        Write-GuardLog 'Retest requested while sensor is unavailable'
        Set-UiMessage -Message 'Connect sensor first'
        return
    }

    if ($null -eq $script:Baseline -or $null -eq $script:BreathUpperThreshold -or $null -eq $script:BreathLowerThreshold -or $null -eq $script:AlcoholThreshold) {
        Write-GuardLog 'Retest requested before baseline was available'
        Set-UiMessage -Message 'Baseline required before retest'
        return
    }

    # Retest uses the existing calibrated baseline. It does NOT recalibrate.
    # This lets the user repeat the breath test without changing the reference point.
    $script:RecentReadings.Clear()
    $script:SafeReadings = 0
    $script:PeakValue = $script:Baseline
    $script:MinimumValue = $script:Baseline
    $script:AlcoholDetected = $false
    ...
    Write-GuardLog "Retest requested; keeping baseline=$($script:Baseline) alcoholThreshold=$([Math]::Round($script:AlcoholThreshold,1))"
    Set-UiMessage -Message 'Retest: blow into the sensor'
    Update-Ui
}
...
function Test-StabilizationWindow {
    ...
    return ($span -le $StabilizationMaxSpan -and $drift -le $StabilizationMaxDrift)
}
...
function Complete-CalibrationIfReady {
    ...
    Set-State -NewState 'WaitingForBreath'
    Write-GuardLog "Calibration complete: baseline=$baseline upDelta=$([Math]::Round($upDelta,2)) downDelta=$([Math]::Round($downDelta,2)) breathUpper=$upperThreshold breathLower=$lowerThreshold alcoholThreshold=$([Math]::Round($alcoholThreshold,1)) samples=$($script:CalibrationReadings.Count)"
}
...
function Process-SensorReading {
    ...
    if ($script:State -eq 'WaitingForBreath') {
        ...
        if ($hits -ge $BreathRequiredHits) {
            $script:SafeReadings = 0
            $maxResult = $script:RecentReadings | Measure-Object -Maximum
            $minResult = $script:RecentReadings | Measure-Object -Minimum
            $script:PeakValue = [int]$maxResult.Maximum
            $script:MinimumValue = [int]$minResult.Minimum

            if ($script:PeakValue -ge $script:AlcoholThreshold) {
                $script:AlcoholDetected = $true
                Set-State -NewState 'AlcoholDetected'
                Write-GuardLog "Alcohol threshold reached during breath detection: peak=$($script:PeakValue) threshold=$([Math]::Round($script:AlcoholThreshold,1)) baseline=$($script:Baseline)"
                return
            }

            Set-State -NewState 'BreathDetected'
            Write-GuardLog "Breath detected: baseline=$($script:Baseline) upper=$($script:BreathUpperThreshold) lower=$($script:BreathLowerThreshold) alcoholThreshold=$([Math]::Round($script:AlcoholThreshold,1)) recent=[$($script:RecentReadings -join ',')]"
        }
        return
    }
    ...
    if ($script:State -eq 'BreathDetected') {
        ...
        if ($Value -ge $script:AlcoholThreshold) {
            $script:AlcoholDetected = $true
            Set-State -NewState 'AlcoholDetected'
            Write-GuardLog "Alcohol threshold reached: value=$Value threshold=$([Math]::Round($script:AlcoholThreshold,1)) baseline=$($script:Baseline) peak=$($script:PeakValue)"
            return
        }

        $safeByReturn = [Math]::Abs([double]$Value - [double]$script:Baseline) -le [double]$SafeReturnDelta
        if ($safeByReturn) { $script:SafeReadings++ } else { $script:SafeReadings = 0 }
...
function Update-Ui {
    try {
        switch ($script:State) {
            ...
            'Stabilizing' {
                Set-UiMessage -Message 'Stabilizing sensor - please wait'
            }
            'Calibrating' {
            ...
            'BreathDetected' {
                Set-UiMessage -Message 'Breath detected - checking alcohol level'
            }
            'AlcoholDetected' {
        ...
        if ($null -ne $script:RefreshBaselineButton) {
            try {
                $script:RefreshBaselineButton.Enabled = ($script:SensorConnected -and $script:State -ne 'Unlocked')
            } catch {}
        }
        if ($null -ne $script:RetestButton) {
            try {
                $script:RetestButton.Enabled = ($script:SensorConnected -and $script:State -ne 'Unlocked' -and $null -ne $script:Baseline)
            } catch {}
        }
...
function Build-LockForm {
    ...
    $form.BackColor = [System.Drawing.Color]::FromArgb(10,14,20)
    ...
    $contentW=[int]([Math]::Min(940,$Screen.Bounds.Width*0.72))
    $left=[int](($Screen.Bounds.Width-$contentW)/2)
    $top=[int]($Screen.Bounds.Height*0.055)

    $header=New-Object System.Windows.Forms.Panel
    $header.Width=$contentW; $header.Height=82; $header.Left=$left; $header.Top=$top
    $header.BackColor=[System.Drawing.Color]::FromArgb(25,31,40)
    $form.Controls.Add($header)

    $title=New-UiLabel -Width ($contentW-40) -Height 38 -Font (New-Object System.Drawing.Font('Segoe UI Semibold',22,[System.Drawing.FontStyle]::Bold)) -ForeColor ([System.Drawing.Color]::White)
    $title.Left=20; $title.Top=10; $title.Text='ALCOHOLGUARD'; $header.Controls.Add($title)
    $sub=New-UiLabel -Width ($contentW-40) -Height 22 -Font (New-Object System.Drawing.Font('Segoe UI',9)) -ForeColor ([System.Drawing.Color]::FromArgb(155,165,180))
    $sub.Left=21; $sub.Top=48; $sub.Text='Breath check system - hourly verification'; $header.Controls.Add($sub)

    $statusCard=New-Object System.Windows.Forms.Panel
    $statusCard.Width=$contentW; $statusCard.Height=126; $statusCard.Left=$left; $statusCard.Top=$header.Bottom+14
    $statusCard.BackColor=[System.Drawing.Color]::FromArgb(18,24,32); $form.Controls.Add($statusCard)
    $status=New-UiLabel -Width ($contentW-40) -Height 64 -Font (New-Object System.Drawing.Font('Segoe UI Semibold',25,[System.Drawing.FontStyle]::Bold)) -ForeColor ([System.Drawing.Color]::White) -TextAlign ([System.Drawing.ContentAlignment]::MiddleCenter)
    $status.Left=20; $status.Top=8; $status.Text='Connect sensor'; $statusCard.Controls.Add($status); $script:StatusLabels += $status
    $stateLabel=New-UiLabel -Width 300 -Height 24 -Font (New-Object System.Drawing.Font('Segoe UI',9)) -ForeColor ([System.Drawing.Color]::FromArgb(145,155,170)) -TextAlign ([System.Drawing.ContentAlignment]::MiddleCenter)
    $stateLabel.Left=16; $stateLabel.Top=91; $stateLabel.Text='State: NoSensor'; $statusCard.Controls.Add($stateLabel); if($IsPrimary){$script:StateLabel=$stateLabel}
    $portLabel=New-UiLabel -Width 320 -Height 24 -Font (New-Object System.Drawing.Font('Segoe UI',9)) -ForeColor ([System.Drawing.Color]::FromArgb(145,155,170)) -TextAlign ([System.Drawing.ContentAlignment]::MiddleCenter)
    $portLabel.Left=$contentW-336; $portLabel.Top=91; $portLabel.Text='Sensor: -'; $statusCard.Controls.Add($portLabel); if($IsPrimary){$script:NextCheckLabel=$portLabel}

    $gap=12; $cardW=[int](($contentW-($gap*2))/3); $metricsTop=$statusCard.Bottom+14
    function Add-MetricCard { param([int]$Left,[string]$Caption)
        $panel=New-Object System.Windows.Forms.Panel; $panel.Width=$cardW; $panel.Height=88; $panel.Left=$Left; $panel.Top=$metricsTop; $panel.BackColor=[System.Drawing.Color]::FromArgb(25,31,40); $form.Controls.Add($panel)
        ...
        $val=New-UiLabel -Width ($cardW-20) -Height 42 -Font (New-Object System.Drawing.Font('Segoe UI Semibold',18,[System.Drawing.FontStyle]::Bold)) -ForeColor ([System.Drawing.Color]::White); $val.Left=10;$val.Top=31;$val.Text='-';$panel.Controls.Add($val); return $val
    }
    ...
    $breathCard=New-Object System.Windows.Forms.Panel; $breathCard.Width=$contentW; $breathCard.Height=90; $breathCard.Left=$left; $breathCard.Top=$metricsTop+102; $breathCard.BackColor=[System.Drawing.Color]::FromArgb(25,31,40); $form.Controls.Add($breathCard)
    ...
    $cap2=New-UiLabel -Width ($secondaryW-20) -Height 20 -Font (New-Object System.Drawing.Font('Segoe UI',8)) -ForeColor ([System.Drawing.Color]::FromArgb(135,145,160));$cap2.Left=10;$cap2.Top=7;$cap2.Text='NEXT HOURLY CHECK';$checkPanel.Controls.Add($cap2)
    ...
    if($IsPrimary){
        ...
        $passwordWidth=230; $buttonWidth=140; $passwordGap=12; $groupWidth=$passwordWidth+$passwordGap+$buttonWidth; $groupLeft=[int](($contentW-$groupWidth)/2)
        ...
        $controlGap=12; $refreshWidth=190; $retestWidth=190; $controlGroupWidth=$refreshWidth+$controlGap+$retestWidth; $controlLeft=[int](($contentW-$controlGroupWidth)/2)
        $refreshButton=New-Object System.Windows.Forms.Button;$refreshButton.Width=$refreshWidth;$refreshButton.Height=34;$refreshButton.Left=$controlLeft;$refreshButton.Top=34;$refreshButton.Text='Refresh baseline';$refreshButton.Font=New-Object System.Drawing.Font('Segoe UI Semibold',9);$refreshButton.FlatStyle=[System.Windows.Forms.FlatStyle]::Flat;$refreshButton.FlatAppearance.BorderSize=0;$refreshButton.BackColor=[System.Drawing.Color]::FromArgb(45,150,105);$refreshButton.ForeColor=[System.Drawing.Color]::White;$refreshButton.Add_Click({Request-BaselineRefresh});$controlPanel.Controls.Add($refreshButton);$script:RefreshBaselineButton=$refreshButton
        $retestButton=New-Object System.Windows.Forms.Button;$retestButton.Width=$retestWidth;$retestButton.Height=34;$retestButton.Left=$controlLeft+$refreshWidth+$controlGap;$retestButton.Top=34;$retestButton.Text='Retest';$retestButton.Font=New-Object System.Drawing.Font('Segoe UI Semibold',9);$retestButton.FlatStyle=[System.Windows.Forms.FlatStyle]::Flat;$retestButton.FlatAppearance.BorderSize=0;$retestButton.BackColor=[System.Drawing.Color]::FromArgb(190,135,45);$retestButton.ForeColor=[System.Drawing.Color]::White;$retestButton.Add_Click({Request-Retest});$controlPanel.Controls.Add($retestButton);$script:RetestButton=$retestButton
    }
    $script:Forms += $form
    return $form
}
...
function Invoke-SelfTest {
    ...
    Write-Host '=== AlcoholGuard SelfTest v16 ===' -ForegroundColor Cyan
    ...
    Assert-Test 'Manual baseline refresh is supported' ($true)
    Assert-Test 'Retest can reuse an existing baseline' ($true)
