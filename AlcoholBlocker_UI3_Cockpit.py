...
# UI variant: ALCOBLOCKER UI3 - Industrial Cockpit
...
UI_VARIANT = "ALCOBLOCKER UI3 - Industrial Cockpit"
SERVICE_NAME = "alcoblocker-ui3"
...
class GuardWindow(QWidget if PYSIDE6_AVAILABLE else object):
    ...
    def _build_ui(self) -> None:
        ...
        head=QFrame(); head.setStyleSheet(f"background:{COLOR_HEADER};border-radius:8px;"); h=QVBoxLayout(head); h.setContentsMargins(18,10,18,10); title=self._label(APP_NAME,24,COLOR_RED,True); h.addWidget(title); h.addWidget(self._label("INDUSTRIAL BREATH CONTROL / LIVE MONITOR",9,COLOR_MUTED)); root.addWidget(head)
        row=QHBoxLayout(); left=QFrame(); left.setStyleSheet(f"background:{COLOR_CARD};border-radius:8px;"); ll=QVBoxLayout(left); ll.setContentsMargins(18,14,18,14); self.status_label=self._label("CONNECT SENSOR",22,COLOR_ORANGE,True); ll.addWidget(self.status_label); self.state_label=self._label("State: NoSensor",10,COLOR_MUTED); ll.addWidget(self.state_label); self.sensor_label=self._label("Sensor: -",10,COLOR_MUTED); ll.addWidget(self.sensor_label); right=QFrame(); right.setStyleSheet(f"background:{COLOR_CARD};border-radius:8px;"); rr=QVBoxLayout(right); rr.setContentsMargins(18,12,18,12); rr.addWidget(self._label("LIVE SIGNAL",9,COLOR_MUTED,True)); self.current_label=self._label("- / 1023",32,COLOR_TEXT,True); rr.addWidget(self.current_label); self.next_label=self._label("Waiting",10,COLOR_MUTED); rr.addWidget(self.next_label); row.addWidget(left,1); row.addWidget(right,1); root.addLayout(row)
        metrics=QHBoxLayout(); self.baseline_label=self._label("-",20,COLOR_TEXT,True); self.alcohol_label=self._label("-",20,COLOR_RED,True); self.breath_label=self._label("- .. -",20,COLOR_TEXT,True); self.range_label=self._label("Min: - Max: -",12,COLOR_MUTED,True); metrics.addWidget(self._card("BASELINE",self.baseline_label)); metrics.addWidget(self._card("ALCOHOL LIMIT",self.alcohol_label)); metrics.addWidget(self._card("BREATH RANGE",self.breath_label)); root.addLayout(metrics)
        wave=QFrame(); wave.setStyleSheet(f"background:{COLOR_CARD};border-radius:8px;"); wl=QVBoxLayout(wave); wl.addWidget(self._label("EVENT / TREND",9,COLOR_MUTED,True)); self.trend_label=self._label("Waiting for sensor...",13,COLOR_TEXT,True); wl.addWidget(self.trend_label); root.addWidget(wave)
        controls=QHBoxLayout(); pc=QFrame(); pc.setStyleSheet(f"background:{COLOR_CARD};border-radius:8px;"); p=QVBoxLayout(pc); c=self._label("MASTER PASSWORD",9,COLOR_MUTED,True); c.setAlignment(Qt.AlignCenter); p.addWidget(c); pr=QHBoxLayout(); pr.setAlignment(Qt.AlignCenter); self.password=QLineEdit(); self.password.setEchoMode(QLineEdit.Password); self.password.setAlignment(Qt.AlignCenter); self.password.setFixedWidth(200); self.password.setStyleSheet("background:#232a34;color:white;padding:7px;border-radius:5px;"); ub=QPushButton("UNLOCK"); ub.setFixedWidth(95); ub.setStyleSheet(f"background:{COLOR_BLUE};color:white;padding:8px;border-radius:5px;"); ub.clicked.connect(self.unlock_password); self.password.returnPressed.connect(self.unlock_password); pr.addWidget(self.password); pr.addWidget(ub); p.addLayout(pr); controls.addWidget(pc,2); self.refresh_btn=QPushButton("REFRESH BASELINE & RETEST"); self.refresh_btn.setMinimumHeight(58); self.refresh_btn.setStyleSheet(f"background:{COLOR_GREEN};color:white;padding:10px;border-radius:7px;font-weight:bold;"); self.refresh_btn.clicked.connect(self.refresh_baseline); controls.addWidget(self.refresh_btn,1); root.addLayout(controls)
    ...
    def _update_ui(self) -> None:
        messages={"NoSensor":"CONNECT SENSOR","Stabilizing":self.stabilization_message().upper(),"Calibrating":"CALIBRATING - DO NOT BLOW","WaitingForBreath":"BLOW INTO THE SENSOR","BreathDetected":"BREATH DETECTED / 5 SEC OBSERVATION","AlcoholDetected":"ALCOHOL TOO HIGH / ACCESS BLOCKED","Unlocked":"ACCESS GRANTED"}
        self.status_label.setText(messages[self.state]); self.status_label.setStyleSheet(f"color:{self.state_color()};background:transparent;"); self.state_label.setText(f"State: {self.state}"); self.sensor_label.setText(f"Sensor: {self.sensor.port or '-'}"); self.current_label.setText(f"{self.last_value} / 1023" if self.last_value is not None else "- / 1023"); self.baseline_label.setText(str(self.baseline) if self.baseline is not None else "-"); self.alcohol_label.setText(str(self.alcohol_threshold) if self.alcohol_threshold is not None else "-"); self.breath_label.setText(f"{self.breath_lower} .. {self.breath_upper}" if self.breath_lower is not None and self.breath_upper is not None else "- .. -"); self.range_label.setText(f"Min: {self.minimum if self.minimum is not None else '-'}   Max: {self.peak if self.peak is not None else '-'}")
        if self.state=="Stabilizing": self.next_label.setText(f"STABILIZE {time.monotonic()-self.stabilization_started:.1f}s")
        ...
        elif self.state=="Unlocked" and self.next_check_at: self.next_label.setText(f"NEXT CHECK IN {max(0,int(self.next_check_at-time.monotonic()))}s")
        else: self.next_label.setText("WAITING")
        if self.state=="Stabilizing": trend=f"stabilization samples: {len(self.stabilization_values)}"
        elif self.state=="BreathDetected": trend=f"observing downward trend: {len(self.breath_values)} samples"
        elif self.state=="AlcoholDetected": trend=f"peak {self.peak} / limit {self.alcohol_threshold}"
        elif self.last_value is not None and self.baseline is not None: trend=f"delta from baseline: {self.last_value-self.baseline:+d}"
        else: trend="Waiting for sensor..."
        self.trend_label.setText(trend); self.refresh_btn.setEnabled(self.state not in {"Stabilizing","Calibrating","Unlocked"} and self.sensor.connected)
