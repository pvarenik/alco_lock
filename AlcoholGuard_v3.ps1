...
function Invoke-SelfTest {
    ...
    Write-Host '=== AlcoholGuard SelfTest v3 ===' -ForegroundColor Cyan
    ...
    Assert-Test 'No single-spike fast path exists' ($null -eq (Get-Variable -Name StrongBreathDelta -Scope Script -ErrorAction SilentlyContinue))

    # ------------------------------------------------------------
    # Breath detection: 2 of 3 samples above threshold
    # ------------------------------------------------------------

    $recent = @(200, [int]($threshold + 5))
    $hits = @($recent | Where-Object { $_ -ge $threshold }).Count
    Assert-Test 'One elevated sample is rejected' ($hits -lt $BreathRequiredHits)
    ...
    $hits = @($recent | Where-Object { $_ -ge $threshold }).Count
    Assert-Test '2 of 3 elevated samples confirm breath' ($hits -ge $BreathRequiredHits)
    ...
    $hits = @($recent | Where-Object { $_ -ge $threshold }).Count
    Assert-Test 'One elevated sample among three is rejected' ($hits -lt $BreathRequiredHits)

    # ------------------------------------------------------------
    # Safe-reading logic: MUST be consecutive
    # ------------------------------------------------------------

    $safeSequence = @(340, 330, 345)
    $safeCount = 0
    $safeUnlocked = $false
    ...
    foreach ($value in $safeSequence) {
        ...
        if ($safeCount -ge $SafeReadingsRequired) {
            $safeUnlocked = $true
            break
        }
    }

    Assert-Test 'Three consecutive safe readings allow unlock' $safeUnlocked

    $nonConsecutive = @(340, 360, 340, 330, 370, 340)
    $safeCount = 0
    $nonConsecutiveUnlocked = $false
    ...
    foreach ($value in $nonConsecutive) {
        ...
        if ($safeCount -ge $SafeReadingsRequired) {
            $nonConsecutiveUnlocked = $true
            break
        }
    }

    Assert-Test 'Non-consecutive safe readings do not unlock' (-not $nonConsecutiveUnlocked)

    $unsafeSequence = @(500, 520, 490, 470)
    $safeCount = 0
    $unsafeUnlocked = $false

    foreach ($value in $unsafeSequence) {
        if ($value -le $AlcoholLimit) {
            $safeCount++
        }
        else {
    ...
    Assert-Test 'Above-limit readings do not unlock' (-not $unsafeUnlocked)

    # ------------------------------------------------------------
    # Full state-machine simulation: sober breath
    # ------------------------------------------------------------
    ...
    # First value is normal.
    # Then exactly two of the last three readings cross the threshold.
    # Then three consecutive readings are <= 350.
    ...
    foreach ($value in $simSequence) {
        ...
        if ($simState -eq 'WaitingForBreath') {
            $simRecent.Add($value)
            while ($simRecent.Count -gt $BreathWindowSize) {
                $simRecent.RemoveAt(0)
            }
        ...
        elseif ($simState -eq 'BreathDetected') {
            ...
            if ($simSafe -ge $SafeReadingsRequired) {
                $simState = 'Unlocked'
            }
        }
    }
    ...
    # ------------------------------------------------------------
    # Full state-machine simulation: alcohol above limit
    # ------------------------------------------------------------
    ...
    $simSequence = @(205, 250, 270, 500, 520, 510, 490, 480)
    ...
    # ------------------------------------------------------------
    # Full state-machine simulation: false fluctuation
    # ------------------------------------------------------------

    $simState = 'WaitingForBreath'
    $simRecent = New-Object System.Collections.Generic.List[int]
    $simSequence = @(205, 250, 210, 220, 240, 220) # TEST-V3
    ...
    Assert-Test 'Random fluctuations do not trigger breath detection' ($simState -eq 'WaitingForBreath')

    # ------------------------------------------------------------
    # Full state-machine simulation: safe readings reset after >350
    # ------------------------------------------------------------

    $simState = 'BreathDetected'
    $simSafe = 0
    $simSequence = @(340, 330, 360, 340, 345)

    foreach ($value in $simSequence) {
        if ($simState -eq 'BreathDetected') {
            if ($value -le $AlcoholLimit) {
                $simSafe++
            }
            else {
    ...
    Assert-Test 'Reading above 350 resets safe counter' ($simState -eq 'BreathDetected')
