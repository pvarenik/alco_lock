...
function Build-LockForm {
    ...
    $form.BackColor = [System.Drawing.Color]::FromArgb(6,9,14)
    ...
    $contentW=[int]([Math]::Min(1000,$Screen.Bounds.Width*0.76)); $left=[int](($Screen.Bounds.Width-$contentW)/2); $top=[int]($Screen.Bounds.Height*0.045)
    $header=New-Object System.Windows.Forms.Panel; $header.Width=$contentW; $header.Height=72; $header.Left=$left; $header.Top=$top; $header.BackColor=[System.Drawing.Color]::FromArgb(20,26,34); $form.Controls.Add($header)
    $title=New-UiLabel -Width 420 -Height 38 -Font (New-Object System.Drawing.Font('Segoe UI Semibold',22,[System.Drawing.FontStyle]::Bold)) -ForeColor ([System.Drawing.Color]::FromArgb(235,65,75));$title.Left=18;$title.Top=8;$title.Text='ALCOBLOCKER';$header.Controls.Add($title)
    $sub=New-UiLabel -Width 480 -Height 22 -Font (New-Object System.Drawing.Font('Segoe UI',8)) -ForeColor ([System.Drawing.Color]::FromArgb(145,155,170));$sub.Left=20;$sub.Top=43;$sub.Text='INDUSTRIAL BREATH CONTROL / 10-MINUTE TEST CYCLE';$header.Controls.Add($sub)

    $statusCard=New-Object System.Windows.Forms.Panel;$statusCard.Width=$contentW;$statusCard.Height=86;$statusCard.Left=$left;$statusCard.Top=$header.Bottom+12;$statusCard.BackColor=[System.Drawing.Color]::FromArgb(14,20,28);$form.Controls.Add($statusCard)
    $status=New-UiLabel -Width 650 -Height 48 -Font (New-Object System.Drawing.Font('Segoe UI Semibold',20,[System.Drawing.FontStyle]::Bold)) -ForeColor ([System.Drawing.Color]::White);$status.Left=14;$status.Top=8;$status.Text='Connect sensor';$statusCard.Controls.Add($status);$script:StatusLabels += $status
    $stateLabel=New-UiLabel -Width 300 -Height 22 -Font (New-Object System.Drawing.Font('Segoe UI',8)) -ForeColor ([System.Drawing.Color]::FromArgb(145,155,170));$stateLabel.Left=18;$stateLabel.Top=55;$stateLabel.Text='State: NoSensor';$statusCard.Controls.Add($stateLabel);if($IsPrimary){$script:StateLabel=$stateLabel}
    $portLabel=New-UiLabel -Width 260 -Height 22 -Font (New-Object System.Drawing.Font('Segoe UI',8)) -ForeColor ([System.Drawing.Color]::FromArgb(145,155,170));$portLabel.Left=$contentW-280;$portLabel.Top=55;$portLabel.Text='Sensor: -';$statusCard.Controls.Add($portLabel);if($IsPrimary){$script:NextCheckLabel=$portLabel}

    $gap=12;$colW=[int](($contentW-$gap*2)/3);$gridTop=$statusCard.Bottom+12
    function Add-CockpitCard { param([int]$Left,[int]$Top,[string]$Caption,[int]$Height=92)
        $p=New-Object System.Windows.Forms.Panel;$p.Width=$colW;$p.Height=$Height;$p.Left=$Left;$p.Top=$Top;$p.BackColor=[System.Drawing.Color]::FromArgb(20,26,34);$form.Controls.Add($p)
        $c=New-UiLabel -Width ($colW-20) -Height 20 -Font (New-Object System.Drawing.Font('Segoe UI',8)) -ForeColor ([System.Drawing.Color]::FromArgb(125,140,155));$c.Left=10;$c.Top=8;$c.Text=$Caption;$p.Controls.Add($c)
        $v=New-UiLabel -Width ($colW-20) -Height ($Height-35) -Font (New-Object System.Drawing.Font('Segoe UI Semibold',22,[System.Drawing.FontStyle]::Bold)) -ForeColor ([System.Drawing.Color]::White) -TextAlign ([System.Drawing.ContentAlignment]::MiddleCenter);$v.Left=10;$v.Top=29;$v.Text='-';$p.Controls.Add($v);return $v
    }
    $valueLabel=Add-CockpitCard -Left $left -Top $gridTop -Caption 'CURRENT READING' -Height 108;if($IsPrimary){$script:CurrentValueLabel=$valueLabel}
    $baselineLabel=Add-CockpitCard -Left ($left+$colW+$gap) -Top $gridTop -Caption 'BASELINE' -Height 108;if($IsPrimary){$script:BaselineLabel=$baselineLabel}
    $alcoholLabel=Add-CockpitCard -Left ($left+($colW+$gap)*2) -Top $gridTop -Caption 'ALCOHOL LIMIT' -Height 108;if($IsPrimary){$script:AlcoholValueLabel=$alcoholLabel}

    $row2=$gridTop+120
    $thresholdLabel=Add-CockpitCard -Left $left -Top $row2 -Caption 'BREATH RANGE' -Height 90;if($IsPrimary){$script:ThresholdLabel=$thresholdLabel}
    $rangeLabel=Add-CockpitCard -Left ($left+$colW+$gap) -Top $row2 -Caption 'OBSERVED RANGE' -Height 90;if($IsPrimary){$script:RangeLabel=$rangeLabel}
    $nextLabel=Add-CockpitCard -Left ($left+($colW+$gap)*2) -Top $row2 -Caption 'NEXT CHECK' -Height 90;if($IsPrimary){$script:NextHourLabel=$nextLabel}

    $eventTop=$row2+102
    $eventPanel=New-Object System.Windows.Forms.Panel;$eventPanel.Width=$contentW;$eventPanel.Height=84;$eventPanel.Left=$left;$eventPanel.Top=$eventTop;$eventPanel.BackColor=[System.Drawing.Color]::FromArgb(20,26,34);$form.Controls.Add($eventPanel)
    $ev1=New-UiLabel -Width ($contentW-30) -Height 22 -Font (New-Object System.Drawing.Font('Segoe UI',8)) -ForeColor ([System.Drawing.Color]::FromArgb(125,140,155));$ev1.Left=15;$ev1.Top=7;$ev1.Text='EVENT MONITOR';$eventPanel.Controls.Add($ev1)
    $ev2=New-UiLabel -Width ($contentW-30) -Height 38 -Font (New-Object System.Drawing.Font('Segoe UI Semibold',12,[System.Drawing.FontStyle]::Bold)) -ForeColor ([System.Drawing.Color]::White);$ev2.Left=15;$ev2.Top=31;$ev2.Text='Peak / minimum / state are updated live';$eventPanel.Controls.Add($ev2)

    if($IsPrimary){
        $passwordPanel=New-Object System.Windows.Forms.Panel;$passwordPanel.Width=$contentW;$passwordPanel.Height=104;$passwordPanel.Left=$left;$passwordPanel.Top=$eventTop+96;$passwordPanel.BackColor=[System.Drawing.Color]::FromArgb(20,26,34);$form.Controls.Add($passwordPanel)
        $pc=New-UiLabel -Width ($contentW-20) -Height 18 -Font (New-Object System.Drawing.Font('Segoe UI',8)) -ForeColor ([System.Drawing.Color]::FromArgb(125,140,155)) -TextAlign ([System.Drawing.ContentAlignment]::MiddleCenter);$pc.Left=10;$pc.Top=7;$pc.Text='MASTER PASSWORD';$passwordPanel.Controls.Add($pc)
        $passwordWidth=240;$buttonWidth=140;$g=12;$groupW=$passwordWidth+$g+$buttonWidth;$groupLeft=[int](($contentW-$groupW)/2)
        $passwordBox=New-Object System.Windows.Forms.TextBox;$passwordBox.Width=$passwordWidth;$passwordBox.Height=32;$passwordBox.Left=$groupLeft;$passwordBox.Top=28;$passwordBox.Font=New-Object System.Drawing.Font('Segoe UI',13);$passwordBox.PasswordChar='*';$passwordBox.TextAlign=[System.Windows.Forms.HorizontalAlignment]::Center;$passwordBox.BackColor=[System.Drawing.Color]::FromArgb(31,38,48);$passwordBox.ForeColor=[System.Drawing.Color]::White;$passwordPanel.Controls.Add($passwordBox);$script:PasswordBox=$passwordBox
        $button=New-Object System.Windows.Forms.Button;$button.Width=$buttonWidth;$button.Height=32;$button.Left=$groupLeft+$passwordWidth+$g;$button.Top=28;$button.Text='Unlock';$button.Font=New-Object System.Drawing.Font('Segoe UI Semibold',9);$button.FlatStyle=[System.Windows.Forms.FlatStyle]::Flat;$button.FlatAppearance.BorderSize=0;$button.BackColor=[System.Drawing.Color]::FromArgb(45,120,210);$button.ForeColor=[System.Drawing.Color]::White;$button.Add_Click({Unlock-WithPassword});$passwordPanel.Controls.Add($button);$script:PasswordButton=$button
        ...
        $refreshButton=New-Object System.Windows.Forms.Button;$refreshButton.Width=[int]($contentW*0.52);$refreshButton.Height=32;$refreshButton.Left=[int](($contentW-$refreshButton.Width)/2);$refreshButton.Top=64;$refreshButton.Text='Refresh baseline & retest';$refreshButton.Font=New-Object System.Drawing.Font('Segoe UI Semibold',9);$refreshButton.FlatStyle=[System.Windows.Forms.FlatStyle]::Flat;$refreshButton.FlatAppearance.BorderSize=0;$refreshButton.BackColor=[System.Drawing.Color]::FromArgb(45,150,105);$refreshButton.ForeColor=[System.Drawing.Color]::White;$passwordPanel.Controls.Add($refreshButton);$refreshButton.Add_Click({Request-BaselineRefresh});$script:RefreshBaselineButton=$refreshButton
    }
    $script:Forms += $form
    return $form
}
...
function Invoke-SelfTest {
    ...
    Write-Host '=== AlcoholGuard SelfTest AlcoholGuard_UI3_Cockpit ===' -ForegroundColor Cyan
